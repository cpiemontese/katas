...
///
/// A way to sell more policies is “upselling” our current customers:
/// offering them additional products that we know they may be interested in, based on the data we have at our disposal.
///
/// Imagine we recently obtained access to an external datasource that we can use to query all the vehicles owned by a
/// particular person. This allows us to find which of our current customers have additional vehicle(s) that are not insured
/// with us. These are great upsell opportunities!
///
/// Your objective is, given a list of some of our current policies (and therefore current customers), to make use of this
/// external API in the most efficient way possible in order to find such upselling opportunities.
///
/// There is a service available that takes a list of person ids, and returns a list of all the vehicles owned by anyone in
/// that list. You can supply up to 100 person ids per call.  Keep in mind that this function wraps an external API, so
/// calls to it are costly and relatively slow.
///
/// # Pseudocode
/// ```text
...
/// (You can obtain this info by calling get_owned_vehicles)
/// In this case, since the only upsell opportunity is to offer a policy to our customer P1 for their other vehicle V3,
/// this is what your function should return:
/// ```
